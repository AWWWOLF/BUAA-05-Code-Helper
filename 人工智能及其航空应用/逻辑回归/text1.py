...
if __name__ == "__main__":
    ...
    theta0 = 100#设置theta0 theta1 theta2的初始值为100、0、0
    theta1 = 0
    theta2 = 0
    ...
    while (abs(temp0) >= 0.001) or (abs(temp1) >= 0.001) or (abs(temp1) >= 0.001):#梯度下降法，判定值为0.001
        ...
        for i in range(0, len(X)):
            for n in range(0, len(X)):#设置步长为0.0000042187
                H = sig(theta.T@X[n])
                temp0 = temp0 + 0.0000042187 * (1 / len(X)) * (H - Y[n])
                temp1 = temp1 + 0.0000042187 * (1 / len(X)) * (H - Y[n]) * X1[n]
                temp2 = temp2 + 0.0000042187 * (1 / len(X)) * (H - Y[n]) * X2[n]
            theta0 = theta0 - temp0
            theta1 = theta1 - temp1
            theta2 = theta2 - temp2
        theta = [theta0, theta1, theta2]
    ...
    y = (theta1 * x + theta0) / (-theta2)#把结果用直线方程表示
